...
export type SemanticMemoryEditorMode = 'edit' | 'preview' | 'split';

export function SemanticMemoryTopbar({
    canSave,
    editorMode,
    inspectorOpen,
    isSaving,
    ...
    isSaving: boolean;
    onEditorModeChange: (mode: SemanticMemoryEditorMode) => void;
    ...
    return (
        <header className="grid h-12 shrink-0 grid-cols-[minmax(0,1fr)_auto] items-center border-border/70 border-b">
            <div className="min-w-0 px-4">
                {pagePath ? (
                    <SemanticMemoryPathBreadcrumb onSelectPath={onSelectPath} path={pagePath} />
                ) : null}
            </div>
            {pageSelected ? (
                <div className="flex items-center justify-end gap-3 py-0 pr-[10px] pl-4">
                    ...
                    <Tabs
                        onValueChange={(value) =>
                            onEditorModeChange(value as SemanticMemoryEditorMode)
                        }
                        value={editorMode}
                    >
                        <TabsList>
...
function SemanticMemoryPathBreadcrumb({
    onSelectPath,
    path,
}: {
    onSelectPath: (path: string) => void;
