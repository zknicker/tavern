...
import type {
    SemanticMemoryMoveTarget,
    SemanticMemoryRenameTarget,
...
interface SemanticMemoryPageFileTreeProps {
    folders: string[];
...
export function SemanticMemoryPageFileTree({
    folders,
    onCreate,
    onDelete,
    onMove,
...
function SemanticMemoryTreeContextMenu({
    context,
    item,
    model,
    onCreate,
    ...
    const target = toSemanticMemoryDeleteTarget(item);
...
function handleRename(
    ...
    onRenamePath({
        ...
        toPath: event.isFolder
            ...
            : toSemanticMemoryPagePath(event.destinationPath),
    });
}

function buildSemanticMemoryFileTreePaths(pages: SemanticMemoryPageNode[], folders: string[]) {
...
function toSemanticMemoryDeleteTarget(item: FileTreeContextMenuItem): SemanticMemoryDeleteTarget {
    const path =
...
function toTreePagePath(path: string) {
    return normalizeSemanticMemoryPath(path);
}

function toSemanticMemoryPagePath(path: string) {
    const normalized = normalizeSemanticMemoryPath(path);
...
function fromTreePath(path: string) {
    return isTreeFolderPath(path) ? fromTreeFolderPath(path) : toSemanticMemoryPagePath(path);
}

function fromTreeFolderPath(path: string | null) {
    return path ? trimTreeFolderSlash(normalizeSemanticMemoryPath(path)) : '';
}

function normalizeSemanticMemoryPath(path: string) {
