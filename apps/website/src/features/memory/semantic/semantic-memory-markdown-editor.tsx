...
import { SemanticMemoryEditorToolbar } from './semantic-memory-mdx-toolbar.tsx';
import {
    fromMdxEditorMarkdown,
    toMdxEditorMarkdown,
} from './semantic-memory-memory-link-markdown.ts';
import './semantic-memory-markdown-editor.css';

interface SemanticMemoryMarkdownEditorProps {
    className?: string;
    disabled?: boolean;
...
export function SemanticMemoryMarkdownEditor({
    className,
    disabled = false,
    onChange,
    onSave,
    saveDisabled,
    value,
}: SemanticMemoryMarkdownEditorProps) {
    ...
    const plugins = React.useMemo(
        () => [
            toolbarPlugin({
                toolbarContents: () => <SemanticMemoryEditorToolbar />,
            }),
    ...
    return (
        ...
        >
            <MDXEditor
                className="semantic-memory-mdx-editor min-h-0 flex-1"
                contentEditableClassName="semantic-memory-mdx-content"
