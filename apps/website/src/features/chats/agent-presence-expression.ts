...
export type AgentPresenceChatRow = TranscriptRow;

export function resolveAgentPresenceExpression(input: {
...
export function getAgentPresenceLabel(emotion: AgentEyeEmotion) {
    switch (emotion) {
        case 'confused':
...
function findLatestTurnProgressRow(rows: AgentPresenceChatRow[], activeReply: ChatActiveReply) {
...
function expressionForRow(row: AgentPresenceChatRow): AgentEyeEmotion {
...
function rowBelongsToActiveRun(row: AgentPresenceChatRow, runId: string) {
...
function isActiveProgressRow(row: AgentPresenceChatRow) {
...
function isFailedProgressRow(row: AgentPresenceChatRow) {
    if (row.kind === 'tool') {
...
function isAssistantProgressRow(row: AgentPresenceChatRow) {
    return (
        row.kind === 'message' ||
        (row.kind === 'system' &&
