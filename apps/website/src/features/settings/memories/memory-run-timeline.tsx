...
export function MemoryRunTimeline({
    jobs,
    onSelectRun,
}: {
...
function RunDot({
    ...
    return (
        <Tooltip
            content={
                <span className="block text-meta">
                    <span className="font-medium">{activityName(run.kind)}</span>
                    {' · '}
        ...
        >
            <button
                aria-label={`${activityName(run.kind)} run, ${runStatusLabel(run.status)}`}
            ...
            >
                <span
                    className={cn(
                        ...
                        runStatusDotClassName(run.status)
                    )}
                />
            </button>
        </Tooltip>
