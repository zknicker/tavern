...
import { WorkerReportContent } from './worker-report-sections.tsx';

export function WorkerReportDrawer({
    jobId,
    onClose,
}: {
    jobId: string | null;
...
function DrawerBody({ jobId }: { jobId: string }) {
    ...
    const job = jobQuery.data as WorkerReportView;

    return (
        <>
            <DrawerHeader className="gap-1 pe-28">
                <DrawerTitle className="text-lg">{workerName(job.kind)}</DrawerTitle>
            ...
            <DrawerPanel className="space-y-4">
                ...
                <WorkerReportContent job={job} />
            </DrawerPanel>
        </>
    );
}

function ReportHeader({ job }: { job: WorkerReportView }) {
            ...
            <div className="flex min-w-0 items-center gap-2 text-sm">
                <span
                    ...
                    className={cn(
                        ...
                        workerStatusDotClassName(job.status)
                    )}
                />
                <p className="font-medium text-foreground">{workerStatusLabel(job.status)}</p>
                {duration ? (
                    <>
...
function durationOf(job: WorkerReportView): number | null {
