...
export type WikiPathDialogMode = 'folder' | 'page';

export interface WikiPathDialogState {
    mode: WikiPathDialogMode;
    parentPath?: string;
    title: string;
}

export function WikiPathDialog({
    errorMessage,
    isPending,
    onClose,
    onSubmit,
    ...
    state: WikiPathDialogState | null;
}) {
    ...
    return (
        ...
        >
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>{state?.title ?? 'Wiki path'}</DialogTitle>
                    <DialogDescription>
                        {state?.parentPath ? `Under ${state.parentPath}.` : 'At the Wiki root.'}
                    </DialogDescription>
                </DialogHeader>
                <form
                    className="space-y-3"
...
export interface WikiDeleteTarget {
    kind: 'folder' | 'page';
    path: string;
    title: string;
}
                ...
                <DialogHeader>
                    ...
                    <DialogDescription>
                        {target?.kind === 'folder'
                            ...
                            : 'This deletes the Markdown page from the Wiki.'}
                    </DialogDescription>
                </DialogHeader>
                {errorMessage ? (
                    <p className="text-destructive-foreground text-sm">{errorMessage}</p>
