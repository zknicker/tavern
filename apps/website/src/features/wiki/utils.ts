import type { WikiListOutput } from '../../lib/trpc.tsx';
import type { WikiPageNode } from './types.ts';

export type WikiPageTreeNode = WikiPageTreeDirectory | WikiPageTreeFile;

export interface WikiPageTreeDirectory {
...
export interface WikiPageTreeFile {
    id: string;
    kind: 'file';
    name: string;
    page: WikiPageNode;
...
export function filterPages(pages: WikiPageNode[], query: string) {
...
export function resolveSelectedPage(list: WikiListOutput, selected: { path: string } | null) {
    if (selected) {
...
export function pageKey(page: Pick<WikiPageNode, 'path'>) {
    return page.path;
}
...
export function joinWikiPath(parentPath: string | undefined, childPath: string) {
...
export function getWikiDirectoryIds(nodes: WikiPageTreeNode[]) {
    return nodes.flatMap((node): string[] => {
        if (node.kind === 'file') {
            return [];
        }
        return [node.id, ...getWikiDirectoryIds(node.children)];
    });
}
...
function finalizeTree(nodes: WikiPageTreeNode[]): WikiPageTreeNode[] {
    return nodes
        .map((node) => {
            if (node.kind === 'file') {
                return node;
...
function compareTreeNodes(left: WikiPageTreeNode, right: WikiPageTreeNode) {
