...
import type { WikiMoveTarget, WikiRenameTarget } from './wiki-page-sidebar.tsx';

type TreeHostStyle = React.CSSProperties & Record<`--${string}`, string>;

interface WikiPageFileTreeProps {
    folders: string[];
...
export function WikiPageFileTree({
    folders,
    onCreate,
    onDelete,
    onMove,
    ...
    readOnly = false,
    selectedPageKey,
}: WikiPageFileTreeProps) {
    const canEdit = !readOnly && onCreate && onDelete && onMove && onRenamePath;
    const treePaths = React.useMemo(() => buildWikiFileTreePaths(pages, folders), [folders, pages]);
...
function WikiTreeContextMenu({
    ...
    const target = toWikiDeleteTarget(item);
...
function handleDropComplete(event: FileTreeDropResult, onMove: (target: WikiMoveTarget) => void) {
...
function handleRename(
    ...
    onRenamePath({
        ...
        toPath: event.isFolder
            ...
            : toWikiPagePath(event.destinationPath),
    });
}

function buildWikiFileTreePaths(pages: WikiPageNode[], folders: string[]) {
...
function toWikiDeleteTarget(item: FileTreeContextMenuItem): WikiDeleteTarget {
    const path =
...
function toWikiPagePath(path: string) {
...
function fromTreePath(path: string) {
    return isTreeFolderPath(path) ? fromTreeFolderPath(path) : toWikiPagePath(path);
}

function fromTreeFolderPath(path: string | null) {
    return path ? trimTreeFolderSlash(normalizeWikiPath(path)) : '';
}
