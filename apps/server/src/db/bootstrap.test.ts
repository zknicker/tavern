...
test('ensureDatabaseSchema adds active turn progress scoped conflict index to existing tables', async () => {
    await withTempDatabase(async (db) => {
        db.exec(`
            CREATE TABLE chat_active_turn_steps (
        ...
        db.prepare(
            `INSERT INTO chat_active_turn_steps (
                run_id, step_id, chat_id, agent_id, session_key, started_at,
                progress_started_at, first_observed_at, updated_at, step_json
            ) VALUES (
                'run-1', 'step-1', 'chat-1', 'agent-1', 'session-1',
                '2026-05-16T12:00:00.000Z', '2026-05-16T12:00:01.000Z',
                '2026-05-16T12:00:01.000Z', '2026-05-16T12:00:01.000Z', '{}'
            )
            ON CONFLICT(session_key, run_id, step_id) DO UPDATE SET
