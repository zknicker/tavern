...
export async function syncAgentRuntimeSessionMessages(input: {
    ...
    const syncedAt = input.syncedAt ?? new Date().toISOString();
    const projectionState = await getSessionMessageProjectionState(input.sessionKey);
...
async function syncSessionsForConnection(input: RuntimeSyncInput) {
    ...
    for (const session of sessions) {
        ...
        try {
            ...
            const projectionState = await getSessionMessageProjectionState(localSessionKey);
            const limit = resolveSessionMessageSyncLimit({
                now: syncedAt,
                projectionState,
            });
...
export async function shouldSyncRuntimeSessionDetails(input: { session: AgentRuntimeSession }) {
    ...
    const projectionState = await getSessionMessageProjectionState(input.session.key);

    if (!(projectionState.hasMessages && projectionState.lastSyncedAt)) {
        return true;
    }
    ...
    const lastSyncedAtMs = Date.parse(projectionState.lastSyncedAt);
...
export function resolveSessionMessageSyncLimit(input: {
    now: string;
    projectionState: { hasMessages: boolean; lastSyncedAt: string | null };
}) {
    if (!(input.projectionState.hasMessages && input.projectionState.lastSyncedAt)) {
        return deepSessionMessageLimit;
    }

    const nowMs = Date.parse(input.now);
    const lastSyncedAtMs = Date.parse(input.projectionState.lastSyncedAt);
...
function mapCronRunProjection(input: {
    jobs: AgentRuntimeCron[];
