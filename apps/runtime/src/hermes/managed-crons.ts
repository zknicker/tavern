...
export const managedCronDefinitions: ManagedCronDefinition[] = [
    {
        ...
        prompt: [
            ...
            'forward. Skip records marked owner: user. When a record turns out to need',
            'human judgment — claim verification, retractions, or paid or private access —',
            'leave it proposed and set owner: user with a clear next_action. If there are no',
    ...
    },
    {
        ...
        prompt: [
            ...
            'references/inventory.md so they surface in normal work. Set owner: user with a',
            'clear next_action only on records a human must decide, such as claim',
            'verification or retraction calls. Do not re-fetch',
            'external sources and do not rewrite article content based on staleness scores',
            'alone. If the hub has no active topic wikis, stop after a one-line summary.',
        ].join(' '),
