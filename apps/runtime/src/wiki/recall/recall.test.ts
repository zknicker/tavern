...
import { recallTurnWiki, recallWikiPages, searchWikiPages } from './recall.ts';
import {
...
describe('Wiki recall', () => {
    ...
    beforeEach(async () => {
        ...
        wikiRoot = await mkdtemp(path.join(tmpdir(), 'tavern-recall-root-'));
        dataDir = await mkdtemp(path.join(tmpdir(), 'tavern-recall-data-'));
        await resetRecallIndexForTesting({
            dbPath: path.join(dataDir, 'wiki-recall.sqlite'),
            wikiRoot,
        });
        await writeFile(
            path.join(wikiRoot, 'hamilton.md'),
            [
                '---',
        ...
        await writeFile(
            path.join(wikiRoot, 'merchbase.md'),
            [
                '---',
    ...
    afterEach(async () => {
        ...
        closeDb();
        await Promise.all([
            rm(wikiRoot, { force: true, recursive: true }),
    ...
    it('finds exact keywords through the lex lane without embeddings', async () => {
        ...
        const result = await searchWikiPages({ query: 'MerchBase royalties' });

        expect(result.hits.map((hit) => hit.path)).toEqual(['wiki/merchbase.md']);
    ...
    it('returns no per-turn recall before embeddings are provisioned', async () => {
        expect(isRecallVectorReady()).toBe(false);
        expect(await recallWikiPages('when is my hamilton show?')).toEqual([]);
        expect(await recallTurnWiki('when is my hamilton show?')).toBeNull();
    });
    ...
    it('reports recall capability as degraded until provisioned', async () => {
        ...
        const [capability] = await refreshRuntimeCapabilities({ ids: ['wikiRecall'] });
    ...
    it.skipIf(!embeddingModelCached)(
        ...
        async () => {
            ...
            let capability = getRuntimeCapability('wikiRecall');
            while (Date.now() < deadline && capability.state !== 'healthy') {
                await new Promise((resolve) => setTimeout(resolve, 25));
                capability = getRuntimeCapability('wikiRecall');
            }
            expect(capability).toMatchObject({
                metadata: { phase: 'ready' },
                state: 'healthy',
    ...
    it.skipIf(!embeddingModelCached)(
        ...
        async () => {
            ...
            await writeFile(
                path.join(wikiRoot, 'new-page.md'),
    ...
    it.skipIf(!embeddingModelCached)(
        ...
        async () => {
            await refreshRecallIndex();
            expect(isRecallVectorReady()).toBe(true);

            const hits = await recallWikiPages('when is my hamilton show?');
            expect(hits[0]?.path).toBe('wiki/hamilton.md');
            expect(hits[0]?.score).toBeGreaterThanOrEqual(0.2);

            const recall = await recallTurnWiki(
                'when is [@Blippy](agent://agt_primary) seeing that theater thing?'
            );
            expect(recall?.block).toContain('Recalled Wiki:');
            expect(recall?.block).toContain('wiki/hamilton.md');
            expect(recall?.hits[0]?.path).toBe('wiki/hamilton.md');
        }
    );
    ...
    it.skipIf(!embeddingModelCached)(
        ...
        async () => {
            await refreshRecallIndex();
            const recall = await recallTurnWiki('completely unrelated quantum festival');
