import { type MemoryJobKind, memoryWorkerStatusListSchema } from '@tavern/api';
...
type WorkerKind = Extract<MemoryJobKind, 'curation' | 'dream' | 'extraction' | 'skill_review'>;
...
const workerKinds: WorkerKind[] = ['extraction', 'dream', 'skill_review', 'curation'];

export function listMemoryWorkers(input: { now?: Date } = {}) {
    const now = input.now ?? new Date();
    return memoryWorkerStatusListSchema.parse({
        workers: workerKinds.map((kind) => {
            const enabled = isWorkerEnabled(kind);
            return {
                enabled,
                kind,
...
function isWorkerEnabled(kind: WorkerKind) {
    if (kind === 'curation') {
    ...
    return (
        isMemoryEnabled() && supportsLanguageModelForRuntime(resolveModelCategorySelection('fast'))
    );
}

function readLastRun(kind: WorkerKind) {
    const row = getDb()
        .prepare(
...
function readNextRun(kind: WorkerKind, now: Date) {
    switch (kind) {
        case 'extraction':
