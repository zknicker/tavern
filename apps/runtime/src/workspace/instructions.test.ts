...
describe('generated agent instructions', () => {
    ...
    test('generates AGENTS.md and seeds empty NOTES.md on first run', async () => {
        registerPlanner();
        ...
        await expect(stat(path.join(workspaceDir, agentWorkDirectoryName))).resolves.toSatisfy(
            (stats) => stats.isDirectory()
        );
    });

    test('tells the agent AGENTS.md is immutable and routes Memory and workspace writes', () => {
        const content = renderAgentInstructions('Planner', '');
        const contentWithoutSkillCli = content.replace(
            /hermes skills search <query> --source skills-sh/gu,
            ''
        );

        expect(contentWithoutSkillCli).not.toMatch(/hermes/iu);
        ...
        expect(content).toContain('For skill add/install/update work');
        expect(content).toContain('`skills_list`');
        expect(content).toContain('`skill_view`');
        expect(content).toContain('`skill_manage`');
        expect(content).toContain('Patch relevant skills before creating new ones');
        expect(content).toContain('`hermes skills search <query> --source skills-sh`');
        ...
        expect(content).toContain('You wake up fresh every time');
        expect(content).toContain('automated memory extraction agent will handle it');
        expect(content).toContain('If the user explicitly asks you to remember');
        expect(content).toContain('use the `memory` skill');
        expect(content).toContain('memory root path through `TAVERN_VAULT_PATH`');
        expect(content).not.toContain('Core files:');
        expect(content).not.toContain('Manual memory edits:');
        expect(content).not.toContain('Extraction pass');
        expect(content).not.toContain('Dreaming pass');
        expect(content).toContain('## Workspace');
        expect(content).toContain('`workbench/`');
        expect(content).toContain('This is your workspace to do your work');
        expect(content).not.toContain('## Vault');
        expect(content).not.toMatch(new RegExp(`\\b${'wi'}${'ki'}\\b`, 'iu'));
    });
    ...
    test('keeps critical operating guidance before large generated sections', () => {
        ...
        expect(content.indexOf('## Workspace')).toBeLessThan(hermesContextHeadChars);
