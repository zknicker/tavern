...
const executionSettingsMetadataKey = 'agent-engine:execution-settings';
...
export async function handleExecutionSettingsRequest(request: Request): Promise<Response | null> {
    ...
    if (request.method === 'GET') {
        return json(agentRuntimeExecutionSettingsSchema.parse(getAgentExecutionSettings()));
    }
    if (request.method === 'PUT') {
        ...
        const settings = saveAgentExecutionSettings(input);
...
export function getAgentExecutionSettings(): AgentRuntimeExecutionSettings {
    const row = getDb()
...
function saveAgentExecutionSettings(
    input: AgentRuntimeSaveExecutionSettings
): AgentRuntimeExecutionSettings {
    const current = getAgentExecutionSettings();
    const next = {
