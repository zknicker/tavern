...
/**
 * One subcommand under a group (e.g. `memory list`, `engine status`). Declares
...
function toCliCommand(groupName: string, sub: SubCommand): CliCommand {
    return {
        ...
        section: groupName === 'engine' ? 'Engine' : groupName === 'memory' ? 'Memory' : 'Status',
        summary: sub.summary,
        usage: sub.usage,
        flags: sub.flags,
    ...
    process.stderr.write(
        `${errorBlock(
            ...
            hint
                ...
                : `Run 'tavern ${groupName} --help' for the command list.`
        )}\n`
    );
    return 2;
}
