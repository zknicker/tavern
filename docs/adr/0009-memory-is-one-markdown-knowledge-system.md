---
summary: Decision to model Tavern Memory as one Markdown knowledge system with per-agent briefings, hidden episodic evidence, and shared Semantic Memory.
read_when:
  - changing Memory, agent briefing files, episodic extraction, dreaming, Semantic Memory, or Memory settings
...
workspace owns that agent's startup briefing files such as `MEMORY.md` and `USER.md` plus hidden
per-agent Episodic Memory. There is no shared `USER.md` or shared `MEMORY.md`. Product copy, docs,
app surfaces, routes, and agent instructions use Memory for briefings, episodic observations,
semantic knowledge pages, indexing, and background maintenance.

Memory has three write layers. Agent briefing files are Layer 1 and may be edited by the owning
agent when the user explicitly asks it to remember or change its operating defaults. Agents only
update their own `USER.md` and `MEMORY.md`. Episodic Memory is
...
not routed through extraction; the main agent should write the appropriate Layer 1 briefing file or
...
The app's primary Memory page exposes Layer 3 Semantic Memory only. Agent briefing files belong in
...
Memory worker activity is hidden from normal chat timelines. Extraction and dreaming appear in the
agent Memory settings page with status, model, usage, costs, file changes, and links back to the
source session or chat. Tavern should model this settings surface after Aside's Memory settings:
a Memory file/settings navigation column, a Memory History list, a detail pane for the selected job,
an explicit Dream now action, and a worker transcript/tool view showing the memory worker's messages
and file-read/write tool calls.
...
follow fixed briefing-file rules from the agent system prompt.

Memory knowledge remains file-backed Markdown, but Runtime bookkeeping lives in SQLite. Agent
briefing files, per-agent episodic Markdown, shared Semantic Memory pages, and the shared
...
Memory workers are Runtime-owned background jobs. Extraction is deterministic over settled
user-facing chat messages. Dreaming uses Runtime model-category resolution and a restricted Semantic
Memory tool set only, with no chat tools, no normal skill set, no SOUL/personality prompt, and no
user-facing response behavior.

V1 has one global Memory on/off setting. When Memory is off, Runtime does not inject briefing files,
...
Normal agents do not need separate confirmation to write Semantic Memory or their own briefing files
...
Layer 1 only when Memory is enabled, remove shared root briefing assumptions, keep the app Memory
