...
skill and installs the managed copy, while `merchbase:merchbase` remains
available through `skill_view`. Other skills under `HERMES_HOME/skills` remain
...
backup unit across Tavern Runtime updates and Hermes pin changes. Updating those
skills is normal Hermes agent-managed skill work: the agent edits or merges the
source copy, and Runtime only refreshes the visible inventory after Hermes
observes the change.
