...
- Hermes participates in those Tavern surfaces rather than redefining them.
...
- Tavern App renders the best Runtime-backed state immediately from cache, then reconciles with
  Tavern Runtime.
- Hermes config and execution evidence appear through Tavern Runtime records.
- Runtime records track freshness.
- If a full Hermes config snapshot omits a runtime-native record, Tavern removes that current
  record.
- Hermes history such as sessions, transcripts, logs, and run events remains useful as
...
- Hermes-native records such as sessions, turns, transcripts, logs, tools, and agent files live
  in Hermes.
- Tavern separately observes and stores what happened in Hermes as execution evidence.

## Runtime Relationship

- Hermes is the managed execution layer connected to Tavern Runtime.
- Hermes behavior maps cleanly into Tavern's named primitives rather than redefining them.
- Tavern Runtime manages Hermes Gateway for the supported local product path.
